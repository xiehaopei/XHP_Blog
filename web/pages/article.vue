<template>
  <div class="container">
    ...
    <section class="list">
      <div class="year-list" v-if="Object.keys(articleList.data).length == 0">空无一物，就像你我一样。</div>
      <template v-else>
        <div class="year-list" v-for="(val, key, idx) in articleList.data" :key="idx">
  ...
  methods: {
    ...
    async loadMoreData() {
      ...
      const { data: res } = await this.$axios.get('article', {
        params: { from: 'list', page: this.page }
      });
      ...
      if (res.status == 1) {
        setTimeout(() => {
          ...
          Object.keys(result).map((item) => {
            const [child_A, child_B] = [this.articleList.data[item], result[item]];
            if (child_A) {
              Object.keys(child_B).map(
                (i) =>
                  (this.articleList.data[item][i] = child_A[i]
                    ? child_A[i].concat(child_B[i])
                    : child_B[i])
              );
            } else {
              this.articleList.data[item] = child_B;
            }
          });
